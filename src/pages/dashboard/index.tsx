...
const UserDisplayComponent: FC<UserDisplayProps> = ({ user }) => {
  ...
  const changeStatus = trpc.user.changeUserStatus.useMutation({
...
const Home: NextPage = () => {
  ...
  const { data: verifiedUsers } = trpc.user.allApprovedUsers.useQuery();

  const { data: rejectedUsers } = trpc.user.allRejectedUsers.useQuery();
  ...
  return (
    <Layout title={"Dashboard"}>
      <div className="mx-auto flex w-11/12 flex-col gap-10 pt-6 text-blue-500">
        ...
        {/*Admin Page*/}
        {ctxUser.type === "ADMIN" && (
          <div className="flex w-1/2 flex-col gap-4 2xl:w-2/5">
            <h2 className="text-2xl font-medium text-gray-200">Admin Panel</h2>
            ...
            <div className="flex flex-col gap-4">
              {pendingUsers &&
                pendingUsers.result.map((user: any) => {
                  if (user.status === "PENDING") {
                    return <UserDisplayComponent key={user.id} user={user} />;
                  }
                  return <></>;
                })}
            </div>
            <h3 className="text-xl font-medium text-gray-200">Active Users</h3>
            <div className="flex flex-col gap-4">
              {verifiedUsers &&
            ...
            <h3 className="text-xl font-medium text-gray-200">Banned Users</h3>
            <div className="flex flex-col gap-4">
              {rejectedUsers &&
