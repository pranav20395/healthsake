...
const Register: NextPage = () => {
    ...
    if (data && data.result.type !== "ORGANIZATION" && data.result.type !== "ORGANISATION") {
        router.push('/');
    }
    ...
    const {register, handleSubmit, formState: {errors}} = useForm<OrgVerifySchema>({
    ...
    const orgVerify = trpc.user.orgSubmitForVerification.useMutation({onSuccess: () => router.reload()});
    ...
    const onSubmit = useCallback(
        async (data: OrgVerifySchema) => {
            ...
            const urlData = {
                ...
                image2: imgUrl2.url,
                location: locUrl.url,
                license: licenseUrl.url,
                permit: permitUrl.url,
                phone: data.phone,
            };
            ...
            {!didSubmit ? (<form className="flex flex-col gap-6"
                ...
                >
                    <label className="flex flex-col gap-2 text-sm">
                        Register as:
                        <select className="py-2 px-2 rounded-lg text-black" {...register("role")}>
                    ...
                    <label className="flex flex-col gap-2 text-sm">
                        ...
                        <input
                            ...
                            type="file" {...register("image1")}/>
                        {errors.image1 &&
                            <p className="text-xs text-red-500">{JSON.stringify(errors.image1.message)}</p>}
                    </label>
                    <label className="flex flex-col gap-2 text-sm">
                        Upload Image 2:
                        <input
                            className="py-2 px-2 rounded-lg text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"
                            type="file" {...register("image2")}/>
                        {errors.image2 &&
                            <p className="text-xs text-red-500">{JSON.stringify(errors.image2.message)}</p>}
                    </label>
                    <label className="flex flex-col gap-2 text-sm">
                        Upload License:
                        <input
                            className="py-2 px-2 rounded-lg text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"
                            type="file" {...register("license")}/>
                        {errors.license &&
                            <p className="text-xs text-red-500">{JSON.stringify(errors.license.message)}</p>}
                    </label>
                    <label className="flex flex-col gap-2 text-sm">
                        Upload Permit:
                        <input
                            className="py-2 px-2 rounded-lg text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"
                            type="file" {...register("permit")}/>
                        {errors.permit &&
                            <p className="text-xs text-red-500">{JSON.stringify(errors.permit.message)}</p>}
                    </label>
                    <label className="flex flex-col gap-2 text-sm">
                        Upload Address Proof:
                        <input
                            className="py-2 px-2 rounded-lg text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"
                            type="file" {...register("location")}/>
                        {errors.location &&
                            <p className="text-xs text-red-500">{JSON.stringify(errors.location.message)}</p>}
                    </label>
                    <label className="flex flex-col gap-2 text-sm">
                        Enter Phone Number:
                        <input className={"py-2 px-2 rounded-lg text-black"} type={"number"} {...register("phone")}/>
                        {errors.phone &&
