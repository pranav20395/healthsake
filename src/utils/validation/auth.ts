...
export const signUpOTPSchema = logSchema.extend({
  fname: z.string().max(20),
  lname: z.string().max(20),
...
export const orgOTPSignUpSchema = logSchema.extend({
  name: z.string().max(20),
  description: z.string().max(200),
  otp: z.string().min(6).max(6),
});
...
export type ITPSignUp = z.infer<typeof signUpOTPSchema>;
export type OSignUp = z.infer<typeof orgSignUpSchema>;
export type OTPSignUp = z.infer<typeof orgOTPSignUpSchema>;
